...
class mosselas5_CACF:
    ...
    def start_tracking(self):
        ...
        #patch_for_DL_training = self._get_subwindow(init_img_training, pos, target_sz)
        ...
        datagen = ImageDataGenerator(rotation_range=12, horizontal_flip=0.7, width_shift_range=[-12,12])
        datagen.fit(patch_for_DL_training_dim_change)
        #save the generated training images
        index_img_test=0
        batch_size=20
        ...
        ##################################################################
        #select all patches other than foreground
        #input_frame_without_target = self._computation_input_frame_without_target(init_img, init_gt, window_sz) 
        #loc11 = input_frame_without_target.shape[0]
        #loc21 = input_frame_without_target.shape[1]
        #overlap_row1 = int(np.ceil(np.divide(window_sz[0],2)))
        #overlap_col1 = int(np.ceil(np.divide(window_sz[1],2)))
        #ind_DL_context1 = 1 
        #for loc_val1 in range(0,loc11,overlap_col1):
            #for loc_col_val1 in range(0,loc21,overlap_row1):
                #location1 =[loc_val1, loc_col_val1]
                #Multiple_patches_from_context_train = self._get_subwindow(input_frame_without_target, location1, window_sz)
                #Multiple_patches_from_context_train = Multiple_patches_from_context_train.astype(np.uint8)
                #cv2.imwrite(os.path.join(train_background, 'Context_patch'+str(ind_DL_context1) +'.jpg'), Multiple_patches_from_context_train)
                #ind_DL_context1 = ind_DL_context1 +1
        ####################################################################
        ...
        for idx in range(len(self.frame_lists)):
            if idx == 0:
                ...
                patch_from_correlation_for_evaluation = self._get_subwindow(current_frame, pos, window_sz)
                ...
                if pred1[0][1] < 0.4:
                    ...
                    loc1 = images.shape[0]
                    loc2 = images.shape[1]
                    overlap_row = int(np.ceil(np.divide(window_sz[0],2)))
                    overlap_col = int(np.ceil(np.divide(window_sz[1],2)))
                    context_patches_around =[]
                    starting_point_y = np.int(np.floor(np.subtract(previous_position[0],window_sz[0])))
                    starting_point_x = np.int(np.floor(np.subtract(previous_position[1],window_sz[1])))
                    ending_point_y = np.int(np.floor(np.add(previous_position[0],window_sz[0])))
                    ending_point_x = np.int(np.floor(np.add(previous_position[1],window_sz[1])))
                    ...
                    for loc_val in range(starting_point_x,ending_point_x,overlap_col):
                        for loc_col_val in range(starting_point_y,ending_point_y,overlap_row):
                            location_x_centrepoint = np.ceil(np.add(loc_val,np.divide(window_sz[1],2)))
                            location_y_centrepoint = np.ceil(np.add(loc_col_val,np.divide(window_sz[0],2)))
                            location =[location_y_centrepoint, location_x_centrepoint]
                            Multiple_patched_from_context_DL = self._get_subwindow(images, location, window_sz)
    ...
    def iou_computation_CACFmethod(self, val):  
        ...
        for i in range(0, len(ground_values)): 
            ...
            ci = [max(c1[0], c2[0]), min(c1[1],c2[1]), min(c1[2],c2[2]), min(c1[3],c2[3])]
            wi = ci[2]-ci[0]
            hi = ci[3]-ci[1]
    ...
    def _resize_img(self, init_gt, init_frame_old):
        if ((np.sqrt(np.multiply(init_gt[2],init_gt[3]))) >= 100):
            ...
            init_gt[2] = np.abs(np.divide(init_gt[2],2))
            init_gt[3] = np.abs(np.divide(init_gt[3],2))
            target_sz = [np.floor(init_gt[3]), np.floor(init_gt[2])]
